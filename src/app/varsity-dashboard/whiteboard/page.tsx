...
function VarsityStudentWhiteboardPage() {
  const router = useRouter();
  const [sessionId, setSessionId] = React.useState('');

  const handleStartSession = () => {
    const newSessionId = nanoid(10);
    router.push(`/varsity-dashboard/whiteboard/${newSessionId}`);
  };

  const handleJoinSession = () => {
    if (sessionId.trim()) {
        router.push(`/varsity-dashboard/whiteboard/${sessionId.trim()}`);
    }
  };
...
export default withAuth(VarsityStudentWhiteboardPage, ['varsity-student']);
